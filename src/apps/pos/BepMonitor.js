...
class BepMonitor extends React.PureComponent{
    constructor(props){
        super(props);
        this.state={
            load:0,
    ...
    async loadWaitingProducts(ma_kho){
        ...
        try{
            ...
            let products = pbls.map(p=>p.details.filter(d=>d.sp_yeu_cau_che_bien).map(d=>{
                    d.pbl = p;
                    return d;
                }).filter(p=>(p.sl_xuat||0)<(p.sl_gui_bep||0)))
    ...
    async finishTable(n_pbl,options={alert:true}){
        ...
        if(pbl){
            pbl.details.filter(d=>d.sp_yeu_cau_che_bien && d.sl_gui_bep>d.sl_xuat).forEach(product=>{
                this.addFinish(product,product.sl_gui_bep-product.sl_xuat,false);
            })
    ...
    renderByTable(){
        ...
        return (
            <Grid container spacing={1}>
                 {pbls.filter(p=>p.ma_ban && p.details.find(d=>d.sl_xuat<d.sl_gui_bep)).sort((a,b)=>{
                            ...
                            <Card style={{width:"100%",height:"100%",padding:0}}>
                                <CardContext style={{padding:0}}>
                                    ...
                                    <List container spacing={1} style={{padding:0}}>
                                        {pbl.details.filter(d=>d.sp_yeu_cau_che_bien).sort((a,b)=>{
                                            let a_sl_con_lai = a.sl_gui_bep -a.sl_xuat;
                                            let b_sl_con_lai = b.sl_gui_bep -b.sl_xuat;
                                                            ...
                                                            <div>
                                                                <div style={{display:"flex",alignItems:"center"}}>
                                                                    <Typography component="div" style={{flex:1}}>
                                                                        {product.sl_xuat}/{product.sl_gui_bep} - {product.ten_vt}
                                                                    </Typography>
                                                                    {product.sl_xuat<product.sl_gui_bep && 
                                                            ...
                                                            {product.sl_xuat<product.sl_gui_bep && 
                                                                <div style={{marginTop:5}}>
                                                                    <Tooltip title={this.context.apis.getLabel("Hoàn thành 1 món")}>
                                                                        <Button size="small" color="secondary" variant="outlined"  disabled={product.sl_xuat>=product.sl_gui_bep} onClick={()=>this.addFinish(product,1)}>
                                                                            <DoneIcon/>
                                                                        </Button>
                                                                    </Tooltip>
                                                                    <Tooltip title={this.context.apis.getLabel("Hoàn thành tất cả các món")}>
                                                                        <Button size="small" color="secondary" variant="outlined"   disabled={product.sl_xuat>=product.sl_gui_bep}  style={{marginLeft:10}}  onClick={()=>this.addFinish(product,product.sl_gui_bep-product.sl_xuat)}>
                                                                            <DoneAllIcon/>
                                                                        </Button>
                                                                    </Tooltip>
                                                                </div>
    ...
    renderByProduct(){
        ...
        return (
            <>
                 {products.filter(p=>p.sl_xuat<p.sl_gui_bep).map(product=>{
                    return (
                                ...
                                <Grid container spacing={1}>
                                    ...
                                    <Grid item style={{display:"flex",alignItems:"center",padding:0}}>
                                        <Typography style={{fontWeight:500}}>
                                            {this.context.apis.getLabel("Hoàn thành")} {product.sl_xuat}/{product.sl_gui_bep}
                                        </Typography>
                                        <Tooltip title={this.context.apis.getLabel("Hoàn thành 1 món")}>
                                            <Button variant="outlined" color="secondary" disabled={product.sl_xuat>=product.sl_gui_bep} style={{marginLeft:10}} onClick={()=>this.addFinish(product,1)}>
                                                <DoneIcon/>
                                            </Button>
                                        </Tooltip>
                                        <Tooltip title={this.context.apis.getLabel("Hoàn thành tất cả các món")}>
                                            <Button variant="outlined"  color="secondary"  disabled={product.sl_xuat>=product.sl_gui_bep}  style={{marginLeft:10}}  onClick={()=>this.addFinish(product,product.sl_gui_bep-product.sl_xuat)}>
                                                <DoneAllIcon/>
                                            </Button>
                                        </Tooltip>
                                    </Grid>
                ...
                <Frag title={this.context.apis.getLabel("Các món đang chờ")} actions={
                    <>
                        <Button onClick={this.pickKho}>
                            {this.context.apis.getLabel("Bếp")}: {(kho||{}).ten_kho|| this.context.apis.getLabel("Chọn cửa hàng")}
                        </Button>
...
BepMonitor.contextType = AuthContext;
BepMonitor.propTypes={
  match: PropTypes.any,
  mediaQueryMatches: PropTypes.bool,
  history:PropTypes.any,
}
class BepMonitorPage extends React.PureComponent{
  componentDidMount(){}
  render(){
    return (
        <Container requireLogin  {...this.props}  showDrawerIfIsDesktop={false}>
            <BepMonitor {...this.props}/>
        </Container>
    )
  }
}
BepMonitorPage.contextType = AuthContext;
export default withRouter(withMediaQuery('(max-width:480px)')(BepMonitorPage));